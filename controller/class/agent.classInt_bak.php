...
function combo()
{
	...
	*/	
	...
	function consulta_personal(){
		if($this->con->conectar()==true){
			return mysql_query("SELECT * FROM personal ORDER BY id_personal ASC");
		}
	}
	...
	function edit_doc($id_documentos,$para,$asunto,$elaboro,$no_folio_asig,$firma,$f_entrega,$tipos,$ori_copia, $status){
		if($this->con->conectar()==true){
	    return mysql_query("UPDATE documentos SET para='".$para."', asunto='".$asunto."', elaboro='".$elaboro."', no_folio_asig='".$no_folio_asig."', firma='".$firma."', f_entrega='".$f_entrega."', ori_copia='".$ori_copia."',status='".$status."' WHERE id_documentos =".$id_documentos)or die( mysql_error() );
		}
	}
	...
	if($this->con->conectar()==true){
	return mysql_query("INSERT INTO documentos (id_documentos,para, asunto, elaboro, no_folio_asig, firma, f_entrega, tipos, ori_copia, status) VALUES (NULL, '".$para."', '".$asunto."', '".$elaboro."', '".$no_folio_asig."', '".$firma."', '".$f_entrega."', '".$tipos."', '".$ori_copia."', '".$status."')")or die( mysql_error() );
	  }
	}
