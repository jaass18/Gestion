...
function combo()
{
	...
	*/	
	...
	function insertar_user($nombre,$a_paterno,$a_materno,$depart,$user,$pass,$nivel){
		if($this->con->conectar()==true){
			return mysql_query("INSERT INTO sis_usuarios (id_usuario,nombre,a_paterno,a_materno,departamento,usuario,password,t_usuario) VALUES (null,'".$nombre."', '".$a_paterno."' ,'".$a_materno."', '".$depart."','".$user."','".$pass."',".$nivel.")");
		}
	}
	
	///////////////////////////////////////Julio/////////////////////////////////////////
	function insertar_personal($nombre,$id_ct,$id_dep_ct,$a_paterno,$a_materno,$curp,$e_mail, $id_puesto, $num_empleado){
		if($this->con->conectar()==true){
			return mysql_query("INSERT INTO sis_personal (id_personal,nombre,id_ct,id_dep_ct,a_paterno,a_materno,curp,e_mail,id_puesto,num_empleado) VALUES (null,'".$nombre."', '".$id_ct."' ,'".$id_dep_ct."', '".$a_paterno."','".$a_materno."','".$curp."','".$e_mail."','".$id_puesto."','".$num_empleado."')");
		}
	}
	...
	function consulta_personal(){
		if($this->con->conectar()==true){
			return mysql_query("SELECT * FROM sis_personal ORDER BY id_personal ASC");
		}
	}
		///////////////////////////////////Julio////////////////////////////////////////////// 
	...
	function consulta_user(){
		if($this->con->conectar()==true){
			return mysql_query("SELECT * FROM sis_usuarios ORDER BY id_usuario ASC");
		}
	}
	...
	function eliminar_personal($iduser){
		if($this->con->conectar()==true){
			return mysql_query("DELETE FROM sis_personal WHERE id_personal =".$idpersonal)or die( mysql_error() );
		}
	}
...
/////////////////////////////////////Julio//////////////////////////////////////////// 
function mostrar_personal($id){
		if($this->con->conectar()==true){
			return mysql_query("SELECT * FROM sis_personal WHERE id_personal=".$id);
		}
	}
/////////////////////////////////////Julio//////////////////////////////////////////// 	
	...
	function edit_personal($nombre,$id_ct,$id_dep_ct,$a_paterno,$a_materno,$curp,$e_mail, $id_puesto, $num_empleado){
		if($this->con->conectar()==true){
	    return mysql_query("UPDATE sis_documentos SET nombre='".$nombre."', id_ct='".$id_ct."', id_dep_ct='".$id_dep_ct."', a_paterno='".$a_paterno."', a_materno='".$a_materno."', curp='".$curp."', e_mail='".$e_mail."',id_puesto='".$id_puesto."',num_empleado='".$num_empleado."' WHERE id_personal =".$id_personal)or die( mysql_error() );
		}
	}
